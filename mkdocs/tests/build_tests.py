...
class BuildTests(PathAssertionMixin, unittest.TestCase):
    ...
    def test_skip_missing_theme_template(self, mock_build_template, mock_write_file):
        ...
        with self.assertLogs("mkdocs") as cm:
            build._build_theme_template(
                "missing.html", env, Files([]), cfg, mock.Mock()
            )
        self.assertEqual(
            "\n".join(cm.output),
    ...
    @tempdir()
    @mock.patch(
        "mkdocs.structure.files.open", mock.mock_open(read_data="template content")
    )
    ...
    @mock.patch(
        "mkdocs.structure.files.open", mock.Mock(side_effect=OSError("Error message."))
    )
    ...
    def test_conflicting_readme_and_index(self, site_dir, docs_dir):
        cfg = load_config(
            docs_dir=docs_dir, site_dir=site_dir, use_directory_urls=False
        )
    ...
    def test_anchor_warning_and_query(self, site_dir, docs_dir):
        cfg = load_config(
            docs_dir=docs_dir, site_dir=site_dir, validation={"anchors": "info"}
        )
    ...
    def test_plugins_adding_files_and_interacting(self, tmp_dir, site_dir, docs_dir):
        def on_files_1(files: Files, config: MkDocsConfig) -> Files:
            ...
            files.append(
                File("SUMMARY.md", tmp_dir, config.site_dir, config.use_directory_urls)
            )
            return files
...
class _TestExtension(markdown.extensions.Extension):
    ...
    def extendMarkdown(self, md: markdown.Markdown) -> None:
        md.preprocessors.register(
            _TestPreprocessor(self.base_path), "mkdocs_test", priority=32
        )
