...
class RelativePathExtensionTests(unittest.TestCase):
    def get_rendered_result(
        ...
        fs = [
            File(f, cfg.docs_dir, cfg.site_dir, cfg.use_directory_urls) for f in files
        ]
        pg = Page("Foo", fs[0], cfg)

        with mock.patch(
            "mkdocs.structure.files.open", mock.mock_open(read_data=content)
        ):
            pg.read_source(cfg)
        if logs:
    ...
    def test_relative_image_link_from_subpage(self):
        self.assertEqual(
            self.get_rendered_result(
                content="![image](../image.png)",
                files=["sub2/non-index.md", "image.png"],
            ),
    ...
    def test_possible_target_uris(self):
        ...
        test(
            ("foo.md", "index.md"),
            exp_true="index.md, foo/index.md",
            exp_false="index.md",
        )
        ...
        test(
            ("foo.md", "foo/bar.md"),
            exp_true="foo/bar.md, foo/foo/bar.md",
        ...
        test(
            ("foo/index.md", "foo/bar.html"),
            expected="foo/foo/bar.html, foo/foo/bar.md",
        )
