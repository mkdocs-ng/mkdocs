...
config_help = "Provide a specific MkDocs config. This can be a file name, or '-' to read from stdin."
dev_addr_help = (
...
serve_clean_help = "Build the site without any effects of `mkdocs serve` - pure `mkdocs build`, then serve."
commit_message_help = (
...
projects_file_help = "URL or local path of the registry file that declares all known MkDocs-related projects."
...
common_config_options = add_options(
    ...
    click.option(
        "-s", "--strict/--no-strict", is_flag=True, default=None, help=strict_help
    ),
...
@click.option(
    "--livereload/--no-livereload", default=True, help="Enable/disable live reloading."
)
