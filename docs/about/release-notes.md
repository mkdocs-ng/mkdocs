...
* Built-in search no longer replaces the page's global `window.postMessage` function in browsers without Web Worker support, which broke other scripts that use it. That fallback also works again: it no longer needs jQuery (which the `mkdocs` theme doesn't include) and finds its files when the site is served from a subdirectory.
