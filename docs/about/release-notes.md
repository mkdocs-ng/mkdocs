...
* `INHERIT` accepts a list of files, so a configuration can be split into several files without chaining them. Later files override earlier ones, and the primary file overrides all of them. A file that inherits from itself is reported as an error instead of crashing. See [Configuration Inheritance](../user-guide/configuration.md#configuration-inheritance).
...
* `mkdocs serve` now rebuilds the site when an inherited configuration file (see `INHERIT`) changes, not only the primary one.
